...
const TEMPLATES_INTRO = {
  ...
  rules: [
    ...
    "Keep naming consistent across sheets — department names in Staff and Patients should match the Department tab.",
...
const TEMPLATES = [
  {
    ...
    name: "Departments",
    sheet: "Department",
    tagline: "Facility structure — building blocks and departments.",
    description:
    ...
    rules: [
      ...
      "Keep one row per department record.",
      "If a field is optional and not applicable, leave it blank.",
    ],
    columns: [
      { name: "building_block", required: true,  meaning: "Name of building or block where the department/unit is located.",   guidance: 'Enter "Main" if the facility has only one block/building without a specific name.' },
      { name: "department",     required: true,  meaning: "Department name.",                                                  guidance: "Use the official department name as it should appear on the Helix app. Example: Plastic Surgery" },
...

  {
    ...
    columns: [
      ...
      { name: "phone",                  required: false, meaning: "Primary phone number.",                                               guidance: "Use a valid phone number for the staff member. Example: 233209182633" },
      { name: "gender",                 required: true,  meaning: "Gender.",                                                             guidance: "Use a consistent value such as Male, Female or Other." },
      { name: "department",             required: true,  meaning: "Department the staff member belongs to.",                             guidance: "Use a department name that exists in the Department tab. Example: Medicine" },
      { name: "patient_access",         required: true,  meaning: "Indicates whether the staff member should have patient access in the system.", guidance: 'Choose "Yes" or "No".' },
...

  {
    ...
    columns: [
      ...
      { name: "department",              required: true,  meaning: "Department this role belongs to.",                                     guidance: "Use a department name that also exists in the Department tab. Example: Information Technology" },
      { name: "priority",                required: true,  meaning: "Indicates whether a role should be filled at all times or not.",      guidance: 'Choose "Critical" if the role needs to be filled at all times. Choose "Standard" if it does not.' },
      ...
      { name: "escalation",              required: false, meaning: "Escalation path for this role.",                                       guidance: "Enter the next role(s) in the escalation ladder. Critical alerts will automatically be forwarded to these roles if the original recipient does not acknowledge the critical alert after a given period. Separate multiple roles with a semicolon (;). Example: CEO; Medical Director" },
    ],
    examples: [
      ...
      ["Senior Consultant on call, Plastic Surgery", "Senior consultant for reconstructive services", "Surgery", "Critical", "No", "", "Yes", "Head of Department, Plastic Surgery; Medical Director"],
      ["Head of Finance", "", "Administration", "Standard", "Yes", "godwinowusu.1984@gmail.com", "No", ""],
...

  {
    ...
    rules: [
      ...
      "The department, floor, and Unit should align with the facility structure defined in the Department tab.",
      "Use bed_number if the facility tracks bed-level placement.",
    ],
    columns: [
      { name: "first_name",            required: true,  meaning: "First name.",                                                         guidance: "Enter the patient's first/given name. Example: Ama" },
      { name: "last_name",             required: true,  meaning: "Last name.",                                                          guidance: "Enter the patient's surname/family name. Example: Mensah" },
      { name: "middle_name",           required: false, meaning: "Middle name.",                                                        guidance: "Optional. Enter the patient's middle name." },
      ...
      { name: "gender",                required: true,  meaning: "Gender.",                                                             guidance: "Use a consistent value such as Male, Female or Other. Example: Female" },
      { name: "department",            required: true,  meaning: "Department currently responsible for the patient or where the patient is admitted.", guidance: "Use a department name that exists in the Department tab. Example: Surgery" },
      { name: "floor",                 required: true,  meaning: "Floor where the patient is located.",                                 guidance: "Use the same naming convention used in the Department tab. Example: Level 3" },
      { name: "Unit",                  required: true,  meaning: "Unit the patient is currently assigned to.",                          guidance: "Use unit names that match your facility structure where possible." },
      { name: "bed_number",            required: true,  meaning: "Bed number or bed label.",                                            guidance: "Use if the facility tracks beds. Example: 2" },
      { name: "room_number",           required: true,  meaning: "Room where the patient is located.",                                  guidance: "Enter the room number or label for the patient's location." },
    ],
    examples: [
      ["Ama",   "Mensah",  "", "1985-03-12", "MRN-BULK-FULL-001", "Female", "Surgery",  "Level 3", "ICU", "2", "201"],
      ["Kwesi", "Boateng", "", "1992-07-22", "MRN-BULK-FULL-002", "Male",   "Medicine", "Level 4", "Male Ward", "2", "405"],
      ["Efua",  "Asante",  "", "1978-10-12", "MRN-BULK-FULL-003", "Female", "Neurology", "Level 5", "Ward C", "", "512"],
    ],
  },
];
...
const DATA_UPLOAD_STEPS = [
  ...
  },
  {
    ...
    shortLabel: "Patients",
    intro:
      "Attach the Patients roster — one row per patient with MRN, location (department, floor, Unit), and demographics aligned to your Department tab.",
  },
];
